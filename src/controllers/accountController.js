    ...
    try {
        ...
        if (rows.length == 1) {
            if (rows[0].status == 1) {
                ...
                await connection.execute('UPDATE `users` SET `token` = ? WHERE `email` = ? ', [md5(accessToken), email]);
                return res.status(200).json({
...
const register = async (req, res) => {
    ...
    let { pwd, invitecode, email, phone } = req.body;
    ...
    try {
        ...
        if (check_email.length == 1 && check_email[0].veri == 1) {
            ...
            if (check_i.length == 1) {
                ...
                const sql = "INSERT INTO users SET id_user = ?, name_user = ?, password = ?, plain_password = ?, money = ?, code = ?, invite = ?, ctv = ?, veri = ?, otp = ?, ip_address = ?, status = ?, time = ?, email = ?, phone = ?";
await connection.execute(sql, [id_user, name_user, md5(pwd), pwd, 0, code, invitecode, ctv, 1, otp2, ip, 1, time, email, phone]);
